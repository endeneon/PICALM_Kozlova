...
genes_to_plot <-
  c("DHCR7",
    "HPGD", "FDFT1", "HMGCR", "PLBD1", "IMPA2", "MTMR1", "RETN", "ATP6AP2",
    "LRP5", "CRABP2", "LDLR", "CD74", "VAMP1", "SREBF2", "GBA", "HMGCS1",
    "SQLE", "MSMO1", "PICALM")

#DHCR7, HPGD, FDFT1, HMGCR, PLBD1, IMPA2, MTMR1, RETN, ATP6AP2,
#LRP5, CRABP2, LDLR, CD74, VAMP1, SERBF2, GBA, HMGCS1, SQLE, MSMO1,PICALM
length(genes_to_plot)
sum(rownames(df_edgeR_DGE) %in% genes_to_plot)
genes_to_plot[!(genes_to_plot %in% rownames(df_edgeR_DGE))]
...
pca_cpm <-
  prcomp(t(as.matrix(cpm_2_plot)),
         center = T,
         scale. = T)
sum(unlist(dimnames(pca_cpm$rotation)[1]) %in% genes_to_plot)
# pca_cpm$x
...
load("Alena_batch_02_03_only_rep_03Oct2023_best_results_so_far_05Oct2023.RData")
...
df_master_4_DGE_combat_sub <-
  df_master_4_DGE_combat[rownames(df_master_4_DGE_combat) %in%
                         ...
                         colnames(df_master_4_DGE_combat) %in%
                           df_metadata_4_plot$sample_names]
sum(rownames(df_master_4_DGE_combat_sub) %in% genes_to_plot)
# df_master_4_DGE_combat <- df_master_4_DGE
...
df_plot_volcano <-
  exp_table
sum(df_plot_volcano$plot_genes)
sum(rownames(df_plot_volcano) %in% genes_to_plot)

df_plot_volcano$`-log10P` <-
  0 - log10(df_plot_volcano$PValue)

df_plot_volcano$significance <-
  "FDR >= 0.05"
df_plot_volcano$significance[df_plot_volcano$FDR < 0.05] <-
  "FDR < 0.05"
#
# genes_to_plot <-
#   c("PICALM",
...
df_plot_volcano$plot_genes <- ""
df_plot_volcano$plot_genes[rownames(df_plot_volcano) %in% genes_to_plot] <-
  rownames(df_plot_volcano)[rownames(df_plot_volcano) %in% genes_to_plot]
length(unique(df_plot_volcano$plot_genes))
# unique(df_plot_volcano$plot_genes)


ggplot(df_plot_volcano,
       aes(x = logFC,
           y = `-log10P`,
           colour = ifelse(test = df_plot_volcano$FDR < 0.05,
                           yes = "FDR < 0.05",
                           no = "FDR >= 0,05"),
           label = plot_genes)) +
  geom_point(size = 0.5) +
...
save.image("Alena_volcano_plot_08Oct2023.RData")


## export all FDR < 0.05 gene names for MAGMA annotation #####
MAGMA_exp_table <-
  exp_table
MAGMA_exp_table <-
  MAGMA_exp_table[MAGMA_exp_table$FDR < 0.05, ]
MAGMA_exp_table$direction <-
  ifelse(test = (MAGMA_exp_table$logFC > 0),
         yes = "up_regulated",
         no = "down_regulated")
MAGMA_exp_table$Gene_symbol <-
  rownames(MAGMA_exp_table)

MAGMA_exp_table <-
  MAGMA_exp_table[, c(7, 1:6)]
MAGMA_exp_table_writeout <-
  MAGMA_exp_table[, c(1, 7)]

write.table(MAGMA_exp_table_writeout,
            file = "Alena_PICALM_risk_v_nonrisk_gene_list_4_MAGMA.txt",
            quote = F, sep = "\t",
            row.names = F, col.names = F)

## broaden gene list to FDR < 0.1
## export all FDR < 0.05 gene names for MAGMA annotation #####
MAGMA_exp_table <-
  exp_table
MAGMA_exp_table <-
  MAGMA_exp_table[MAGMA_exp_table$FDR < 0.1, ]
MAGMA_exp_table$direction <-
  ifelse(test = (MAGMA_exp_table$logFC > 0),
         yes = "up_regulated",
         no = "down_regulated")
...
write.table(MAGMA_exp_table_writeout,
            file = "Alena_PICALM_risk_v_nonrisk_gene_list_4_MAGMA_FDR_0.1.txt",
            quote = F, sep = "\t",
            row.names = F, col.names = F)


## export 3-group all gene list table (up/nonsig/down)
MAGMA_exp_table <-
  exp_table

MAGMA_exp_table$direction <- "non-significant"
MAGMA_exp_table$direction[(MAGMA_exp_table$logFC > 0) &
                            (MAGMA_exp_table$FDR < 0.05)] <- "up-regulated"
MAGMA_exp_table$direction[(MAGMA_exp_table$logFC < 0) &
                            (MAGMA_exp_table$FDR < 0.05)] <- "down-regulated"

MAGMA_exp_table$Gene_symbol <-
  rownames(MAGMA_exp_table)

MAGMA_exp_table_writeout <-
  MAGMA_exp_table[, c(7, 6)]

write.table(MAGMA_exp_table_writeout,
            file = "Alena_PICALM_risk_v_nonrisk_gene_full_list_3_groups_4_MAGMA.txt",
            quote = F, sep = "\t",
            row.names = F, col.names = F)
