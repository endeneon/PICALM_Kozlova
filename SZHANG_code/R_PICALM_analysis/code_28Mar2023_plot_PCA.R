...
View(unlist(cpm_gene_count[rownames(cpm_gene_count) == "GAPDH", ]))

View(unlist(df_4_DGE[rownames(df_4_DGE) == "PICALM", ]))
View(unlist(df_4_DGE[rownames(df_4_DGE) == "GAPDH", ]))

library_size <- colSums(df_4_DGE)
names(library_size) <- colnames(df_4_DGE)
View(library_size)
...
# View(as.numeric(colSums(cpm_gene_count)))
...
fviz_pca_ind(df_PCA_to_plot,
             ...
             palette = "aaas",
             invisible = "quali",
             title = "PICALM_rs10792832_2000_genes_no_combat")
...
write.table(df_to_plot,
            file = "PICALM_risk_vs_non_risk_28Mar2023.tsv",
            quote = F, sep = "\t",
            row.names = T, col.names = T)
...
write.table(df_to_plot,
            file = "PICALM_CRISPRoff_vs_non_risk_28Mar2023.tsv",
            quote = F, sep = "\t",
            row.names = T, col.names = T)
...
write.table(df_to_plot,
            file = "PICALM_CD09_CRISPRoff_vs_non_risk_28Mar2023.tsv",
            quote = F, sep = "\t",
            row.names = T, col.names = T)
