...
ibed_5kb_all_files <-
  list.files(path = "/home/zhangs3/Data/FASTQ/microC/brain_20Nov2019_HiC/hg19_2_hg38",
             pattern = ".*_hg38.ibed$",
             full.names = T,
             recursive = T)
  ...
  for (j in 1:length(ibed_5kb_ranges)) {
    ...
    df_2_make_ranges <-
      read_delim(ibed_5kb_all_files[j],
                 ...
                 escape_double = FALSE,
                 trim_ws = TRUE)
    colnames(df_2_make_ranges) <-
      c("bait_chr", "bait_start", "bait_end", "bait_name",
        "otherEnd_chr", "otherEnd_start", "otherEnd_end", "otherEnd_name",
        "N_reads", "score")
    df_2_make_ranges <-
      df_2_make_ranges[str_detect(string = df_2_make_ranges$bait_name,
                                  pattern = "bait",
                                  negate = F), ]
