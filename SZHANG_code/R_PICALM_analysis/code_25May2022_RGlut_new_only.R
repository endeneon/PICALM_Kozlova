...
# load data
ASoC_df_raw <- 
  read_delim("DP20_data_files/non_500bp_intersected/NGN2_RGlut_merged_41_lines_SNP_in_peaks_03Jun2022_DP20_4_R.txt", 
...
ASoC_df_table <- ASoC_df[, -c(7:88)]


## write out the table
write.table(ASoC_df_table, file = "DP20_data_files/RGlut_41_lines_500bp_SNP_262K_06Jun2022.txt",
...
write.table(ASoC_df_table, 
            file = "RGlut_41_lines_SNP_FDR005_annovar_06Jun2022.txt",
            row.names = F, col.names = T, sep = "\t", quote = F)
...
# load data
ASoC_df_raw <- 
  read_delim("two_batchs_merge/RGlut_41_lines_merged_SNP_26May2022_DP20_4_R.txt", 
             delim = "\t", escape_double = FALSE, 
             trim_ws = TRUE)
...
## write out the table
write.table(ASoC_df_table, file = "DP20_data_files/RGlut_41_lines_SNP_929K_25May2022.txt",
            row.names = F, col.names = T, sep = "\t", quote = F)
...
## write out table for annovar
write.table(ASoC_df_table, 
            file = "RGlut_41_lines_SNP_FDR005_annovar_26May2022.txt",
            row.names = F, col.names = T, sep = "\t", quote = F)
