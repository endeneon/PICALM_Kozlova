...
# load data
ASoC_df_raw <- 
  read_delim("DP20_data_files/non_500bp_intersected/MG_28_lines_merged_peaks_filtered_03Jun2022_DP_20_4_R.txt", 
...
ASoC_df_table <- ASoC_df[, -c(7:62)]

sum(ASoC_df$binom.P < 0.05)
sum(ASoC_df$FDR < 0.05)

## write out the table
write.table(ASoC_df_table, file = "DP20_data_files/MG_28_lines_SNP_361KK_06Jun2022.txt",
...
write.table(ASoC_df_table, 
            file = "MG_28_lines_SNP_500bp_FDR005_annovar_06Jun2022.txt",
            row.names = F, col.names = T, sep = "\t", quote = F)
