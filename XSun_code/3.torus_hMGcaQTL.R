...
for (i in 1:length(file_gwas)) {
  ...
  annotation_bed_files <- "data/hMGcaQTL.bed"
  
  torus.files <- prepare_torus_input_files(gwas, annotation_bed_files, torus_input_dir = "./torus_input1")
  ...
  tmp1 <- torus.enrich[torus.enrich$term == "hMGcaQTL.bed.1", ]
  tmp1$snp <- "hMGcaQTL"
  tmp1[,1] <- trait
  
  enrichment <- rbind(enrichment,tmp1)
  save(enrichment,file = "enrichment_hMGcaQTL.rdata")
}
