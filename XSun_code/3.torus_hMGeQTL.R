...
for (i in 6:length(file_gwas)) {
  ...
  annotation_bed_files <- "data/hMGeQTL.bed"
  
  torus.files <- prepare_torus_input_files(gwas, annotation_bed_files, torus_input_dir = "./torus_input2")
  ...
  tmp1 <- torus.enrich[torus.enrich$term == "hMGeQTL.bed.1", ]
  tmp1$snp <- "hMGeQTL"
  tmp1[,1] <- trait
  
  enrichment <- rbind(enrichment,tmp1)
  save(enrichment,file = "enrichment_hMGeQTL.rdata")
}
