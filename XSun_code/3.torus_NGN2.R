...
for (i in 1:length(file_gwas)) {
  ...
  annotation_bed_files <- "data/NGN2.bed"
  
  torus.files <- prepare_torus_input_files(gwas, annotation_bed_files, torus_input_dir = "./torus_input4")
  ...
  tmp1 <- torus.enrich[torus.enrich$term == "NGN2.bed.1", ]
  tmp1$snp <- "NGN2"
  tmp1[,1] <- trait
  
  enrichment <- rbind(enrichment,tmp1)
  save(enrichment,file = "enrichment_NGN2.rdata")
}
